    ...
    for api_response in api_responses:    
        ...
        for pod in api_response.items:
            ...
            if r.exists(f"regex_cmp:{regexsha}:{logid}:{pod.metadata.namespace}:{pod.metadata.name}"):
                ...
                if re.search(r"{pod_re}", pod.metadata.name) or re.search(r"{pod_re}", pod.metadata.name) or (pod_re == pod.metadata.name) or (pod_re == pod.metadata.name):
                ...
                for container in container_list:
                    ...
                    if pod.status.phase != "Unknown" and pod.status.phase != "Pending":
                        ...
                        try:
                            ...
                            logging.debug(f"[logid:{logid}][k-inv][logs-loop] pod_start_time_sec is {pod_start_time_sec}")
